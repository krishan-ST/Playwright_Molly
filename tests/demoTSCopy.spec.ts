...
import PersonalInformation from "../Functions/personalinfo";
import Causes from "../Functions/causes";
import SDG from "../Functions/sdg";
...
import CompleteProfile from '../Functions/completeProfile';
import HomeLanding from '../Functions/homeLanding';
...
test.describe.serial('API Testing', () => {
    ...
    test('Login user @reg', async ({ request, page, context }) => {

        const home = new Home(page);
        const personalInformation = new PersonalInformation(page);
        const causes = new Causes(page);
        const sdg = new SDG(page);
        const skills = new Skills(page);
        const language = new Language(page);
        const availability = new Availability(page);
        const completeProfile = new CompleteProfile(page);
        const homeLanding = new HomeLanding(page);

        await home.login(email,pwd);
        await personalInformation.setPersonalInfo('Monica','Geller','10','10','1982','Male');
        await causes.setCauses('Animal Welfare');
        await sdg.setSDG('climate_action','life_water');
        await skills.setSkills('Accounting');
        await language.setLanguages(1,3,4);
        await availability.setAvailability(1,2,3);
        await completeProfile.setCompleteProfile('774455886','Sri Lanka','Colombo','Hi my name is Monica Geller');        
        await homeLanding.assertWormText("Yay... you got the worm!");
